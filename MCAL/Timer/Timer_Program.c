...
void Timer_vidTimer0Init(void)
{
	...
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_WGM00);
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_WGM01);
...
#error "Wrong Timer0 mode configuration !"
	...
	SET_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS00);
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS01);
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS02);
#elif TIMER_TIMER0_PRESCALER == 8
	/* Select Input CLK to Timer0 as MCU CLK divided by 8 Presacler */
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS00);
	SET_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS01);
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS02);
	...
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS00);
	CLR_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS01);
	SET_BIT(TCCR0_REG,TIMER_TIMER0_TCCR0_CS02);
...
u8 Timer_u8Timer0SetBusyWait(u32 Copy_u32DelayTime_ms)
{
	...
	if(Copy_u32DelayTime_ms < 4294967296)
	{
		...
		u32 Local_u32Timer0RequiredTicks = Copy_u32DelayTime_ms/((f32)1000*Global_f32Timer0TickTime); // Number of required ticks
		u32 Local_u32Timer0OverflowCounter = 0 ;  													  // Timer0 overflow counter

		/* Calculate number of overflows required for desired delay */
		u32 Local_u32Timer0OverflowNum = (f32)Copy_u32DelayTime_ms/((f32)1000*Global_f32Timer0OverflowTime) ;

		/* Set Timer0 Preload Value */
		TCNT0_REG = 256-(Local_u32Timer0RequiredTicks/Local_u32Timer0OverflowNum);
		...
		while(Local_u32Timer0OverflowCounter < Local_u32Timer0OverflowNum)
		{
			...
			while(!GET_BIT(TIFR_REG,TIMER_TIMER0_TIFR_TOV0));
			/* Clear TOV0 Flag by writing 1 to its bit in TIFR Register */
			SET_BIT(TIFR_REG,TIMER_TIMER0_TIFR_TOV0);
...
{
	CLR_BIT(TIMSK_REG,TIMER_TIMER0_TIMSK_TOIE0);
}
/**********************************************************************************/
...
{
	SET_BIT(TIMSK_REG,TIMER_TIMER0_TIMSK_OCIE0);
}
/**********************************************************************************/
...
{
	CLR_BIT(TIMSK_REG,TIMER_TIMER0_TIMSK_OCIE0);
}
/**********************************************************************************/
